...
def handler(q, ix, startSeed, seedRange):
  print(f"Seed #({ix}): {startSeed}, {seedRange}")
  myMinimum = -1
...
if __name__ == '__main__':
  ...
  while ix < lnSeeds:
    ...
    seedRange = seeds[ix]
    ix += 1
    t = Process(target=handler, args=(q, cnt, startSeed, seedRange))
    t.start()
    thrds.append(t)
    cnt += 1
  ...
  for t in thrds:
    r = q.get()
    if Minimum == -1: Minimum = r
    elif r < Minimum: Minimum = r
